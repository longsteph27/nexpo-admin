...
function resolveOptionLabels(field: FormAnswer['field'], rawValue: string): string {
  ...
  return parts.map(v => map.get(v) ?? formatValue(v)).join(', ');
}
...
function parseIdentity(sub: CompanySubmission): { name: string; email: string; phone: string } {
  ...
  // Fallback: try to extract from profile answers
  ...
  return { name: texts.slice(0, 2).join(' ').trim() || 'Unnamed', email, phone };
}
...
function CompanySheet({ sub, open, onClose, eventId }: { sub: CompanySubmission | null; open: boolean; onClose: () => void; eventId: string }) {
  ...
  function getExhibitorLabel(ex: any): string {
    ...
    const n = t?.company_name || ex.representative_name || `Exhibitor ${String(ex.id).slice(0, 8)}`;
    return ex.booth_number ? `${n} (Booth ${ex.booth_number})` : n;
  }
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto px-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center flex-shrink-0">
                <span className="text-sm font-semibold text-emerald-700">{name.charAt(0).toUpperCase()}</span>
              </div>
              <div>
            ...
            {!booking && (
              <button
                onClick={() => setBooking({ exhibitorId: '', scheduledAt: '', notes: '' })}
                className="flex items-center gap-1.5 text-xs font-medium text-emerald-600 border border-emerald-200 rounded-lg px-3 py-1.5 hover:bg-emerald-50 transition-colors flex-shrink-0"
              >
        ...
        {booking && (
          <div className="mb-5 rounded-xl border border-emerald-100 bg-emerald-50/50 p-4 space-y-3">
            <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wide">Book Business Meeting</p>
            <div>
              ...
              <select
                ...
                onChange={e => setBooking(b => ({ ...b!, exhibitorId: e.target.value }))}
              >
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-content-secondary uppercase tracking-wide">Company Profile</span>
          </div>
...
export default function CompanyProfilesPage() {
  ...
  const [selected, setSelected] = useState<CompanySubmission | null>(null);
  const queryClient = useQueryClient();
  ...
  const bulkDeleteMutation = useMutation({
    ...
    onSuccess: (count) => {
      ...
      selection.clear();
      queryClient.invalidateQueries({ queryKey: ['company-profiles', eventId] });
    },
    onError: () => toast.error('Failed to delete profiles'),
  });

  const { data: companyForm } = useQuery({
    queryKey: ['company-form', eventId],
    queryFn: async () => {
      const res = await directus.request(
        readItems('forms', {
          filter: { event_id: { _eq: Number(eventId) }, linked_module: { _eq: 'company_profiles' } } as any,
  ...
  const formId = (companyForm as any)?.id;

  const { data: submissions = [], isLoading } = useQuery({
    queryKey: ['company-profiles', eventId, formId],
    queryFn: async () => {
      const res = await directus.request(
      ...
      return res as CompanySubmission[];
    },
    enabled: !!formId,
  });
  ...
  const formTitle =
    (companyForm as any)?.translations?.find((t: any) => t.languages_code === 'vi-VN')?.title ||
    (companyForm as any)?.translations?.[0]?.title ||
    'Company Profiles';
  ...
  return (
    <Container>
      <ContainerHeader
        title="Companies"
        description={formId ? `Source: ${formTitle}` : 'No company profiles form configured for this event'}
      />
      ...
      {!formId && !isLoading && (
        <div className="flex flex-col items-center justify-center py-16 text-content-secondary">
          ...
          <p className="text-sm font-medium">No company profiles form linked to this event</p>
          <p className="text-xs mt-1">Go to Forms → set Linked Module = Company Profiles (Business)</p>
        </div>
      )}
      ...
      {formId && (
        <>
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs text-content-secondary">{filtered.length} companies</span>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-content-secondary">
              <Icon icon="lucide:building-2" className="w-10 h-10 mb-3 text-gray-300" />
              <p className="text-sm font-medium">No companies yet</p>
              <p className="text-xs mt-1">Companies will appear here after visitors fill the profile form</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold text-content-secondary">Company</th>
