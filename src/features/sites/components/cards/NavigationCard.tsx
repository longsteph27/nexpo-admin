...
import type { FeatureSite, SiteNavigationSummary, SiteNavigationItemSummary } from '../../types';
...
export default function NavigationCard({ site }: NavigationCardProps) {
  const navigationItems: SiteNavigationSummary[] = site?.navigation ?? [];
        ...
        {navigationItems.length > 0 ? (
          <div className="space-y-3">
            {navigationItems.map((nav) => (
              <div key={nav.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
