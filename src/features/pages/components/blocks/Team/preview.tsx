...
export default function TeamBlock({ data, lang }: TeamBlockProps) {
    const directusLang = lang === 'en' ? 'en-US' : 'vi-VN';
    const teams = data.team as TeamMember[] || [];

    // Translation logic for Block
    const translation =
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 mt-12">
                        {teams.map((member, idx) => {
                            ...
                            if (typeof member.image === 'string') {
                                imageUrl = getDirectusMedia(member.image);
                                ...
                                // Fallback if image is just an ID in some contexts
                                imageUrl = getDirectusMedia(member.image as any);
                            }

                            const memberTranslation = member.translations?.find(
                                t => t.languages_code === directusLang
                            ) || member.translations?.[0]; // Fallback
