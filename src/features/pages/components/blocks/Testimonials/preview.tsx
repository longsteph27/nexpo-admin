...
import type { BlockTestimonials, Testimonial } from './types';
...
interface TestimonialsPreviewProps {
    data: BlockTestimonials | Record<string, unknown>;
    lang: string;
}

export default function TestimonialsPreview({ data, lang }: TestimonialsPreviewProps) {
    ...
    // Block subtitle is removed as it's not in the schema (types.ts)

    const testimonials = ((data.testimonials as any[]) || [])
        .map(item => {
            if (typeof item === 'string') return null;
            if (item.testimonials_id && typeof item.testimonials_id === 'object') {
    ...
    const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
        if (!testimonialRefs.current[0]) return;
        ...
        if (scrollLeft === 0) {
            ...
            setCurrentItemIdx(testimonials.length - 1);
        } else {
    ...
    return (
        <BlockContainer className="relative overflow-hidden" fullWidth>
            ...
            <div className="relative mt-8">
                ...
                >
                    {testimonials.map((testimonial, itemIdx) => {
                        ...
                        const testimonialTitle = testimonialTranslation?.title;
                        const testimonialSubtitle = testimonialTranslation?.subtitle;
                        const testimonialContent = testimonialTranslation?.content;

                        const imageUrl = testimonial.image
                            ? assetsApi.getAssetUrl(typeof testimonial.image === 'string' ? testimonial.image : testimonial.image.id)
                            : null;

                        return (
                            <div
                                key={testimonial.id || `idx-${itemIdx}`}
                                ref={el => {
                            ...
                            >
                                ...
                                <div className="mt-6 flex items-center space-x-4 border-t pt-6">
                                    ...
                                    <div className="text-left">
                                        <p className="font-[var(--font-body) ] font-semibold text-lg text-[var(--color-primary)]">
                                            {testimonialTitle}
                                        </p>
                                        <p className="font-[var(--font-body)] text-sm text-gray-500">
                                            {testimonialSubtitle} {testimonial.company ? `at ${testimonial.company}` : ''}
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    {testimonials.length === 0 && (
                        <div className="w-full text-center py-8 text-gray-400 border-2 border-dashed border-gray-200 rounded-lg">
                            <VIcon icon="lucide:message-circle" className="w-8 h-8 mx-auto mb-2 opacity-50" />
