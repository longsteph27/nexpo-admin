...
interface FooterNavigationDialogProps {
  isOpen: boolean;
  onClose: () => void;
  siteId: number;
  ...
  onUpdate?: () => void;
  onSaveChanges?: (items: NavigationItem[], preparedData?: { navigationId: string | null; siteId: number; footerNavigation: any; payload: any }) => void; // Callback to save changes to parent component without API call
}
...
export default function FooterNavigationDialog({
  isOpen,
  onClose,
  siteId,
  footerNavigation,
  ...
  const buildHierarchy = (flatItems: any[]): NavigationItem[] => {
    if (!flatItems || !Array.isArray(flatItems)) return [];
  ...
  // Initialize items from footerNavigation
  useEffect(() => {
    if (isOpen && footerNavigation) {
      if (footerNavigation.items && Array.isArray(footerNavigation.items)) {
        setItems(buildHierarchy(footerNavigation.items));
      } else {
        setItems([]);
      }
    } else {
  ...
  const prepareNavigationData = (itemsData: NavigationItem[]) => {
      ...
      const navigationId = footerNavigation?.id || null; // Will be created if null

      // Step 2: Prepare items for batch operations
      // Get existing item IDs from footerNavigation
      ...
      if (footerNavigation?.items && Array.isArray(footerNavigation.items)) {
        ...
        flattenExisting(footerNavigation.items);
      }
  ...
  const updateItem = (index: number, field: string, value: any, lang?: 'en-US' | 'vi-VN', path?: number[]) => {
    ...
    if (field === 'title') {
      const editLang = lang || itemEditLang[targetItem.id || `item-${index}`] || activeLang;
      const transIndex = targetItem.translations.findIndex(t => t.languages_code === editLang);
      if (transIndex >= 0) {
    ...
    if (path) {
      ...
      let current = updatedItems[path[0]];
      ...
      if (current.children) {
        current.children[path[path.length - 1]] = targetItem;
      }
    ...
    if (path && path.length > 0) {
      ...
      if (current.children) {
        ...
        if (current.children.length === 0) {
          current.has_children = false;
        }
      }
      setItems(newItems);
    } else {
      setItems(items.filter((_, i) => i !== index).map((item, i) => ({ ...item, sort: i })));
    }
    setExpandedItem(null);
  };

  const moveItem = (index: number, direction: 'up' | 'down', path?: number[]) => {
    if (path && path.length > 0) {
      const newItems = [...items];
      let current = newItems[path[0]];
      for (let i = 1; i < path.length - 1; i++) {
      ...
      if (current.children) {
        ...
        if (targetIndex >= 0 && targetIndex < current.children.length) {
          [current.children[childIndex], current.children[targetIndex]] = [current.children[targetIndex], current.children[childIndex]];
          current.children.forEach((child, i) => { child.sort = i; });
        }
      }
      setItems(newItems);
    } else {
  ...
  const renderItemForm = (item: NavigationItem, index: number, path: number[] = []) => {
    ...
    return (
      <div key={itemId} className="border border-neutral-200 rounded-lg p-4 bg-white">
        {/* Item Header */}
        <div
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center space-x-1 bg-neutral-100 rounded-lg p-0.5">
                  <button
                    ...
                    onClick={() => {
                      const itemId = item.id || `item-${index}`;
                      setItemEditLangForItem(itemId, 'en-US');
                    }}
                  ...
                  <button
                    ...
                    onClick={() => {
                      const itemId = item.id || `item-${index}`;
                      setItemEditLangForItem(itemId, 'vi-VN');
                    }}
                    className={`px-2 py-1 text-xs font-medium rounded transition-colors ${
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="px-6 py-4 border-b border-neutral-200">
          <DialogTitle className="flex items-center space-x-3">
            ...
            <span>Edit Footer Navigation</span>
          </DialogTitle>
        </DialogHeader>
