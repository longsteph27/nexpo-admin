...
import { usePagesBySite } from '@/features/pages/hooks/usePages';
...
export default function PagesList({ eventId }: PagesListProps) {
  ...
  const limit = 10;
  const { data: pagesData, isLoading: pagesLoading } = usePagesBySite({
    siteId: Number(site?.id),
    page,
    limit,
  });
  const pages = (pagesData && 'items' in pagesData) ? pagesData.items : [];
  const pagination = (pagesData && 'pagination' in pagesData) ? pagesData.pagination : undefined;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {pages.map((page: any, index: number) => (
            <motion.div
              ...
              onClick={() => router.push(`/events/${eventId}/pages/${page.id}`)}
            >
              <div className="p-6">
