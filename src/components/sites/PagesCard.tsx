...
export default function PagesCard({ site, eventId, siteId }: PagesCardProps) {
  ...
  const getPermalink = (page: any, language: string = 'en') => {
    ...
    const translation = page.translations?.find((t: any) => 
      t.languages_code === (language === 'en' ? 'en-US' : 'vi-VN')
    );
    
    // Return just the permalink endpoint, not the full URL
    return translation?.permalink || '';
  };

  const getFullUrl = (page: any, language: string = 'en') => {
    if (!site?.slug) return '';
    
    const permalink = getPermalink(page, language);
    if (!permalink) return '';
    
    // Build full URL with permalink at the end
    return `https://nxp-public-ruby.vercel.app/${site.slug}/${language}/${permalink}`;
  };

  const copyPermalink = async (page: any, language: string = 'en', event: React.MouseEvent) => {
