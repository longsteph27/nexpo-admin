...
export default function Sidebar({ isOpen, onClose, onToggle, isCollapsed = false }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div className={`hidden lg:block lg:fixed lg:inset-y-0 lg:z-50 transition-all duration-300 ${isCollapsed ? 'lg:w-0' : 'lg:w-80'}`}>
        {/* Toggle Button - always visible */}
        <button
  onClick={onToggle}
  className={`absolute top-1/2 -translate-y-1/2 
    w-6 h-14 bg-nexpo-bg-sidebar text-white 
    rounded-tr-full rounded-br-full 
    shadow-md hover:shadow-lg transition-all duration-200 
    flex items-center justify-center z-10 
    ${isCollapsed ? 'left-0' : '-right-6'}`}
>
  <Icon
  ...
  />
        ...
        {!isCollapsed && (
          <div className="relative flex flex-col h-full bg-nexpo-bg-sidebar text-sidebar-text-primary w-full">
            {/* Header */}
